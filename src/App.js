...
function App() {
  ...
  const saveTasksForUser = (username, nextTasks) => {
    ...
    syncTasksToBackend(username, nextTasks);
  };
  ...
  // Background sync for tasks (non-blocking)
  const syncTasksToBackend = (username, tasksToSync) => {
    if (!username) return;
    fetch(`${TASKS_API}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: username, tasks: tasksToSync }),
    }).catch(() => {});
  };
  ...
  const handleLogin = (event) => {
    ...
    // Fetch tasks from backend to sync from other devices (non-blocking background)
  ...
  const handleSignup = (event) => {
    ...
    // Fetch tasks from backend for new account (non-blocking background)
    fetch(`${TASKS_API}?email=${encodeURIComponent(username)}`)
      .then(r => r.json())
      .then(backendTasks => {
        if (Array.isArray(backendTasks) && backendTasks.length > 0) {
